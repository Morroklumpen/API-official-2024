...
const getMyQuotes = async (req, res) => {
  ...
  if (typeof user !== "undefined") {
    ...
    try {
      feedback = createFeedback(
        200,
        " Quotes found in the database: " + quotes.length,
        true,
        quotes,
        user.username
      );
...
const createquote = async (req, res) => {
  const { quote, author, user } = req.body;
  ...
  if (
    ...
    try {
      ...
      feedback = createFeedback(
        200,
        "Quote was inserted to the database",
        true,
        newQuote,
        user.username
      );
    } catch (err) {
      console.log("ERROR createquote: ", err.message);
